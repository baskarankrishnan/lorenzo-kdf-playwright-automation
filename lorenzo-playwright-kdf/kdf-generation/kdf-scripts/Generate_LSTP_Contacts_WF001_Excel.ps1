# Generate_LSTP_Contacts_WF001_Excel.ps1
# Generates the Excel workbook for LSTP_Contacts_WF001 from the KDF JSON script

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$repoRoot  = Split-Path -Parent (Split-Path -Parent $scriptDir)
$jsonPath  = Join-Path $scriptDir "LSTP_Contacts_WF001.json"
$outDir    = Join-Path $repoRoot "excelFramework\testcases\Contacts"
$outFile   = Join-Path $outDir "LSTP_Contacts_WF001.xlsx"
...
$td_headers = @(
    ...
    "PURPOSE", "CONTACT_TYPE", "CONTACT_SUBJECT", "CONSULTATION_MEDIUM", "LOCATION_TYPE",
    "OUTCOME"
)
...
$td_sample = @(
    ...
    "Community Mental Health", "GP", "Routine", "Internal",
    "Assessment", "Face to face", "Initial Assessment", "Face to Face", "Community",
...
$configData = @(
    ...
    @("Last Updated",          $today),
    @("Status",                "Ready"),
    @("Prerequisites",         "Lorenzo Contacts module enabled, patient registration rights available, referral creation rights available, Care Events EPR view accessible, contact types and purposes configured, document template configured for contacts"),
    @("Test Data Variables",   "FORENAME, CITY, COUNTRY, COUNTRY_OF_BIRTH, NATIONALITY, ETHNICITY, SERVICE_TYPE, REFERRAL_SOURCE, REFERRAL_PRIORITY, REFERRAL_TYPE, PURPOSE, CONTACT_TYPE, CONTACT_SUBJECT, CONSULTATION_MEDIUM, LOCATION_TYPE, OUTCOME"),
    @("Assertions",            "8"),
    ...
    @("Workflows Covered",     "Login + Patient Registration (Full Roadmap 2 Sections) + NHS Allocation + Navigate Back to Patient EPR + Capture PAS ID + Create Referral + Navigate Care Events EPR View + Create Single Contact (Planned) + Validate Planned Status + Select Planned Encounter + Record Contact (Actual) + Validate Actual Status + Select Actual Encounter + Modify Contact (Actual) + Validate Actual Encounter Closed + Validate Encounter List and Details + Validate Patient Event Status + Record Quick Contact (Actual with Referral) + Validate Quick Contact Actual Status + Logout")
)
...
$tvData = @(
    ...
    @("_PASID",         "PAS ID captured from patient EPR view after registration, used for reference",          "Captured at runtime"),
