# Generate_LSTP_NursingActivity_WF001_Excel.ps1
# Generates LSTP_NursingActivity_WF001.xlsx in excelFramework\testcases\NursingActivity\

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Definition
$repoRoot  = Split-Path -Parent (Split-Path -Parent $scriptDir)
$jsonPath  = Join-Path $scriptDir "LSTP_NursingActivity_WF001.json"
$outDir    = Join-Path $repoRoot "excelFramework\testcases\NursingActivity"
$outFile   = Join-Path $outDir  "LSTP_NursingActivity_WF001.xlsx"
...
$dataHeaders = @(
    ...
    "PRESENTING_COMPLAINT","ATTENDANCE_TYPE","ATTENDANCE_SOURCE","CONSULTATION_MEDIUM",
    "TIME_ELAPSED","PATIENT_GROUP","MODE_OF_ARRIVAL",
...
$sampleValues = @(
    ...
    "Chest Pain","Initial Attendance","Walk-in","Face to Face",
    "0-4 hours","Adult","Walking",
    "Allergy","Penicillin","Patient",
    "Clinical Alert","Drug Alert"
)
...
$configData = @(
    @("Test Case ID",           "LSTP_NursingActivity_WF001"),
    @("Module",                 "Nursing Activity"),
    @("Sub-Module",             "ED Attendance with Allergy and Alert Recording"),
    @("Description",            "End-to-end workflow covering ED attendance creation, patient registration, allergy recording, alert recording, and EPR validation for Nursing Activity module"),
    @("Complexity",             "High"),
    @("Priority",               "P1"),
    ...
    @("Last Updated",           $today),
    @("Status",                 "Ready"),
    @("Prerequisites",          "Lorenzo application accessible, ED ward configured, valid care provider exists"),
    @("Test Data Variables",    $datasetCols),
    @("Assertions",             "$assertions"),
    @("Pages Used",             $pagesUsed),
    @("Workflows Covered",      "Login + My Work Navigation + ED Ward Selection + Create Attendance + Patient Registration + NHS Number Capture + Attendance Form Completion + Patient Selection + Allergy Recording + Alert Recording + EPR View + Health Issues Validation + Logout")
)
