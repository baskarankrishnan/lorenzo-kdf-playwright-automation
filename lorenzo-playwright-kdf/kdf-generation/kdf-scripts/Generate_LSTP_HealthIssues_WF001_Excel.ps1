# Generate_LSTP_HealthIssues_WF001_Excel.ps1
# Generates the Excel workbook for LSTP_HealthIssues_WF001 (Health Issues)
# Sheets: TestExecution | TestData | ExecutionConfig | TestValues

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$repoRoot  = Split-Path -Parent (Split-Path -Parent $scriptDir)
$outDir    = Join-Path $repoRoot "excelFramework\testcases\HealthIssues"
if (-not (Test-Path $outDir)) { New-Item -ItemType Directory -Path $outDir | Out-Null }

$outFile   = Join-Path $outDir "LSTP_HealthIssues_WF001.xlsx"
$jsonFile  = Join-Path $scriptDir "LSTP_HealthIssues_WF001.json"
...
$dataHeaders = @(
    "CITY","COUNTRY","WARD_NAME","INTENDED_MANAGEMENT","SERVICE_TYPE",
    "REFERRAL_PRIORITY","REFERRAL_TYPE","REFERRAL_SOURCE",
    "ADMISSION_SOURCE","ADMISSION_TYPE","CARE_PROVIDER_ID",
    "ALERT_TYPE","ALERT_NAME","ALERT_INFO_SOURCE",
    "ALERT_CLOSE_REASON","ALERT_REOPEN_REASON","ALERT_STRIKEOUT_REASON",
    "ALLERGY_TYPE","ALLERGEN","ALLERGY_INFO_SOURCE",
    "ALLERGY_ONSET_DATE","ALLERGY_CLOSE_REASON","ALLERGY_REOPEN_REASON","ALLERGY_STRIKEOUT_REASON"
)
...
# Sample data row 2
$sampleData = @(
    "London","United Kingdom","Ward A","Elective","General Medicine",
    "Routine","GP Referral","GP",
    "Emergency Admission","Elective","DR001",
    "Medical","Allergy","Self",
    "No longer relevant","Patient request","Data entry error",
    "Drug","Penicillin","Patient",
    "01/01/2024","No longer relevant","Patient request","Data entry error"
)
...
$configData = @(
    @("Test Case ID",           "LSTP_HealthIssues_WF001"),
    @("Module",                 "Health Issues"),
    @("Sub-Module",             "Alerts and Allergies/ADRs Lifecycle"),
    @("Description",            "Covers full lifecycle of Alerts (Record, Modify, Close, Re-open, Strike) and Allergies/ADRs (Record, Modify, Close, Re-open, Strike) within Health Issues EPR for an admitted inpatient"),
    @("Complexity",             "High"),
    @("Priority",               "P1"),
    ...
    @("Status",                 "Ready"),
    @("Prerequisites",          "Valid login credentials, Ward A available, IP bed available, Care Provider configured"),
    @("Test Data Variables",    "CITY,COUNTRY,WARD_NAME,INTENDED_MANAGEMENT,SERVICE_TYPE,REFERRAL_PRIORITY,REFERRAL_TYPE,REFERRAL_SOURCE,ADMISSION_SOURCE,ADMISSION_TYPE,CARE_PROVIDER_ID,ALERT_TYPE,ALERT_NAME,ALERT_INFO_SOURCE,ALERT_CLOSE_REASON,ALERT_REOPEN_REASON,ALERT_STRIKEOUT_REASON,ALLERGY_TYPE,ALLERGEN,ALLERGY_INFO_SOURCE,ALLERGY_ONSET_DATE,ALLERGY_CLOSE_REASON,ALLERGY_REOPEN_REASON,ALLERGY_STRIKEOUT_REASON"),
    @("Assertions",             "20"),
    @("Pages Used",             "pageLogin,pageHome,pageInpatient,pagePatientBasicSearch,pagePatientSearch,pageFindandbook,pageIPSMBasicSearchCriteria,pageBookWardappointment,pageReferralDetails,pageIPPegboardCurrentView,pagePatADMAdmission,pageAdmitIPAdmitRoad,pageAdmitIPAdmitRoadCPSFS,pageAdmitIPAdmitRoadCG,pageLORENZO,pageEPRView,pageHistoryTab,pageCsWzModifyRisk,pageReopenRisk,pageRiskListView,pageHIALRRecordallergy,pageLIMainViewResource"),
    @("Workflows Covered",      "Login + Patient Registration + IP Booking + Admission + Health Issues Alerts Full Lifecycle + Allergies/ADRs Full Lifecycle + Logout")
)
...
Write-Host ("  Row 175 (expect 174): " + $ws.Cells.Item(175,1).Value2)
