import * as pageLoaderUtils from './pageLoaderUtils';
import * as path from 'path';
import * as fs from 'fs';

/**
 * Validation script to verify all Lorenzo page files are properly loaded
 * Run: npx ts-node core/utilities/validatePages.ts
 */
...
async function validatePages() {
    ...
    try {
        ...
        const totalElements = Object.values(repository).reduce((sum, page) => sum + Object.keys(page).length, 0);
        ...
        const pageStats = Object.entries(repository)
            .map(([name, elements]) => ({ name, count: Object.keys(elements).length }))
        ...
        for (const [pageName, elements] of Object.entries(repository)) {
            for (const [elementName, locator] of Object.entries(elements)) {
                if (locator.xpath && locator.xpath.trim().startsWith('//')) {
                    validXpaths++;
                } else {
                    invalidXpaths++;
                    console.log(`  ⚠️ Invalid XPath in ${pageName}.${elementName}: ${locator.xpath?.substring(0, 50)}`);
                }
            }
        }
        ...
        return { pageCount, totalElements, avgElements, validXpaths, invalidXpaths };
