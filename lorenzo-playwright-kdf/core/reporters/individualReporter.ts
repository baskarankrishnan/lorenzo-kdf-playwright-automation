...
<h1><span class="title-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg></span>${escapeHtml(testResult.testCaseId)}</h1>
<p>${escapeHtml(testResult.testCaseDescription || 'Test Case Execution Report')}</p>
</div>
<div class="header-right">
<span class="duration-badge"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>${testResult.duration}</span>
...
function generateStepRows(steps: stepExecutionResult[], reportDir: string): string {
    ...
    steps.forEach((step, index) => {
        ...
        rows += `<tr class="${expandableClass}" id="step-${step.stepNo}"><td ${onclickExpand}>${expandIcon}${step.stepNo}</td><td>${escapeHtml(step.stepDescription || '')}</td><td>${escapeHtml(step.actionKeyword)}</td><td style="text-align:center"><button class="icon-button" onclick="showInfoModalById('${stepDataId}')" title="View Details"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg></button></td><td><span class="step-status ${statusClass}">${statusText}</span></td><td>${step.stepDuration}</td><td style="text-align:center"><button class="icon-button" onclick="showScreenshotModal('${screenshotPath}',${step.stepNo})" title="View Screenshot"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg></button></td></tr>`;
        ...
        if (hasPageActions) {
            step.pageActions.forEach((pageAction) => {
                ...
                rows += `<tr class="page-action-row" data-parent="${index}"><td>${pageAction.stepNo}</td><td class="page-action-indicator">↳ ${escapeHtml(pageAction.stepDescription || '')}</td><td>${escapeHtml(pageAction.actionKeyword)}</td><td style="text-align:center"><button class="icon-button" onclick="showInfoModalById('${pageActionDataId}')" title="View Details"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg></button></td><td><span class="step-status ${pageActionStatusClass}">${pageActionStatusText}</span></td><td>${pageAction.stepDuration}</td><td style="text-align:center"><button class="icon-button" onclick="showScreenshotModal('${pageActionScreenshotPath}',${pageAction.stepNo})" title="View Screenshot"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg></button></td></tr>`;
