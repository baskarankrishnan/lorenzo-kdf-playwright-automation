...
export async function resolveElement(page: Page, baseSelector: string, step: testStep, timeout = 30000): Promise<Locator> {
  ...
  if (step.elementText !== null && step.elementText !== undefined) {
    const resolvedElementText = resolveTestVariables(String(step.elementText));
    if (baseSelector.includes('<variable>')) {
      baseSelector = baseSelector.replace('<variable>', resolvedElementText);
    }
  }
