...
import { setTextBox, setAutoCompleteField, clickElement, resolveElement, resolveElements, sleep, clickAndHandleAlert } from "../core/actionkeywords/elementActions";
...
export async function verifyRecordInTable(page: Page, step: testStep): Promise<void> {
  try {
    ...
    let expectedRefs = isMultiColumn
    ...
    const headers = await tableLocator.locator(`xpath =//tr[contains(@id,'g_TRColHdr')]//tbody//td[contains(@id,'igHTD')]`).evaluateAll(elements => {
      // Try to get header text from <span> if present, else from the cell itself
      return elements.map(el => {
